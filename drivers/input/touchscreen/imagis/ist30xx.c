...
static int ist30xx_pinctrl_configure(struct ist30xx_data *data, bool active)
{
	...
	struct pinctrl_state *set_state_id;
	int retval;
	...
	if (active) {
		...
		set_state_id = 
			pinctrl_lookup_state(data->pinctrl,
						"tsp_id_gpio_active");
		if (IS_ERR(set_state_id)) {
			tsp_err("%s: cannot get pinctrl(id) active state\n", __func__);
			return PTR_ERR(set_state_id);
		}
		...
		set_state_id = 
			pinctrl_lookup_state(data->pinctrl,
						"tsp_id_gpio_suspend");
		if (IS_ERR(set_state_id)) {
			tsp_err("%s: cannot get pinctrl(id) sleep state\n", __func__);
			return PTR_ERR(set_state_id);
		}
	}
	...
	retval = pinctrl_select_state(data->pinctrl, set_state_id);
	if (retval) {
		tsp_err("%s: cannot set pinctrl(tsp_id) %s state\n",
				__func__, active ? "active" : "suspend");
		return retval;
	}
...
static void ist30xx_request_gpio(struct i2c_client *client, struct ist30xx_data *data) {
	...
	if (gpio_is_valid(data->dt_data->keyled_en_gpio)) {
		ret = gpio_request(data->dt_data->keyled_en_gpio, "imagis,touch_en_gpio");
		if (ret) {
			tsp_err("%s: unable to request touch_en_gpio [%d]\n",
...
static int ist30xx_parse_dt(struct device *dev, struct ist30xx_data *data)
{
	...
	tsp_info("%s: irq:%d, touch_en:%d, keyled_en:%d, system_rev = %d\n", __func__,
		 data->dt_data->irq_gpio, data->dt_data->touch_en_gpio, data->dt_data->keyled_en_gpio, system_rev);
