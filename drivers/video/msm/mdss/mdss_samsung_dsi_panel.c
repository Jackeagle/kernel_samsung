...
#if defined(CONFIG_ESD_RECOVERY)
	ctrl_pdata->esd_gpio = of_get_named_gpio(np, "qcom,esd-irq-gpio", 0);
...
static void err_fg_work_func(struct work_struct *work)
{
    ...
    if (mfd->panel_power_on && (msd.mfd->resume_state == MIPI_RESUME_STATE)) {
		char *envp[2] = {"PANEL_ALIVE=0", NULL};
		int ret;
		int bl_backup = msd.dstat.bright_level;
		msd.pdata->panel_info.panel_dead = true;
    ...
    pr_info("%s end\n", __func__);
	enable_irq(gpio_to_irq(msd.ctrl_pdata->esd_gpio));
    return;
}
#ifdef ESD_DEBUG
...
static int __init detect_lcd_panel_vendor(char* read_id)
{
	...
	else if (lcd_id == 0x420001 || lcd_id == 0x520001 || lcd_id == 0x500001 || lcd_id == 0x510001)
