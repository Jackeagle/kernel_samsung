...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	...
	&& !defined(CONFIG_FB_MSM_MIPI_SHARP_HD_VIDEO_PANEL) && !defined(CONFIG_MACH_KLEOS_CTC)
...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	if (!no_panel) {
		...
		wmb();
		if (mipi->lp11_init){
#if defined(CONFIG_FB_MSM_MIPI_HIMAX_WVGA_VIDEO_PANEL) || defined(CONFIG_FB_MSM_MIPI_HX8389C_QHD_VIDEO_PANEL) || defined(CONFIG_MACH_KLEOS_CTC)
			mdelay(5);
			if (mdss_dsi_pinctrl_set_state(ctrl_pdata, true))
				pr_debug("reset enable: pinctrl not enabled\n");
#endif		
