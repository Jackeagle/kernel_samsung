...
static int
board_dai_init(struct snd_soc_pcm_runtime *rtd)
{
	return 0;
}
...
static int
dbmd2_snd_probe(struct platform_device *pdev)
{
	int ret = 0;
...
//	struct device_node *np = pdev->dev.of_node;
	...
	if(!is_codec_probe_done()){
		dev_err(&pdev->dev, "%s - defer as codec_probe_done is false\n", __func__);
		return -EPROBE_DEFER;
	}
#endif
...
static int
dbmd2_snd_remove(struct platform_device *pdev)
{
