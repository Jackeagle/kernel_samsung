...
#define USE_FORMATS		(SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_MU_LAW)
...
static u32
stream_get_position(struct snd_pcm_substream *substream)
{
...
static void
stream_set_position(struct snd_pcm_substream *substream, u32 position)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static void
dbmd2_pcm_timer(unsigned long _substream)
{
	struct snd_pcm_substream *substream = (struct snd_pcm_substream *)_substream;
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static int
dbmd2_pcm_open(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct timer_list *timer;
...
static int
dbmd2_stop_period_timer(struct snd_pcm_substream *substream)
{
	struct timer_list *timer = substream->runtime->private_data;
...
static int
dbmd2_pcm_close(struct snd_pcm_substream *substream)
{
	struct timer_list *timer = substream->runtime->private_data;
...
static int
dbmd2_pcm_hw_params(struct snd_pcm_substream *substream,
...
static int
dbmd2_pcm_prepare(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static int
dbmd2_start_period_timer(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static int
dbmd2_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	switch (cmd) {
...
dbmd2_pcm_pointer(struct snd_pcm_substream *substream)
{
	u32 pos;
...
static int
dbmd2_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
...
static int
dbmd2_pcm_probe(struct snd_soc_platform *pt)
{
...
static int
dbmd2_pcm_remove(struct snd_soc_platform *pt)
{
	struct snd_dbmd2 *dbmd2;
...
static int
dbmd2_pcm_new(struct snd_soc_pcm_runtime *runtime)
{
	struct snd_pcm *pcm;
	int ret = 0;
...
static void
dbmd2_pcm_free(struct snd_pcm *pcm)
{
...
static int
dbmd2_pcm_platform_probe(struct platform_device *pdev)
{
	int err;

	err = snd_soc_register_platform(&pdev->dev, &dbmd2_soc_platform);
	if (err)
		printk("snd_soc_register_platform() failed");

	return err;
}

static int
dbmd2_pcm_platform_remove(struct platform_device *pdev)
{
